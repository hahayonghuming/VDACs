...
**Note:** We find that **VDAC**s are sensitive to `v_coef` located in `src/config`. This value penalizes critic losses. In our orignial implementation, we set `v_coef=0.5`. However, we later find out that `v_coef=0.1` yields better performance.
### 1c3s5z
